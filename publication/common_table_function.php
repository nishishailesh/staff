...
//////////////////////////////////////////////////
//Read new data
function add($link,$d,$t,$default=array(),$style='')
{
	...
	echo '<div class="container bg-warning '.$d.'_'.$t.'" '.$style.'>';
...
//search window
function search($link,$d,$t,$default,$hv)
{
	$fld=get_key($link,$d,$t);
	...
	echo '<div class="container bg-light">';
			echo '<div class="row bg-warning">';	
					...
					echo '<input type=hidden name=^hv readonly value=\''.$hv.'\'>';
				...
				echo '<div class="col-sm-12 text-center">';	
					...
					echo '<span class="badge badge-danger border border-dark">Search</span>';
				echo '</div>';
...
function show_search_rows($link,$d,$t,$post,$offset)
{
	echo '<div class="row">';	
		echo '<div class="col text-center">';	
			...
			echo '<input type=hidden name=action value=show_search_rows>';
			echo '<input type=hidden name=^database readonly value=\''.$d.'\'>';
			echo '<input type=hidden name=^table readonly value=\''.$t.'\'>';
			echo '<input type=hidden name=^hv readonly value=\'v\'>';

			echo mk_search_input_from_post($link,$d,$t,$post);		
			echo '</form>';
			}
	...
	$result=get_search_result($link,$d,$t,$post,$offset);
...
function show_search_rows_h($link,$d,$t,$post,$offset)
{
	echo '<div class="row">';	
		echo '<div class="col text-center">';	
			//echo '<button class="btn btn-warning" type=button onclick="showHideClass(\'hdn\')">Toggle show</button>';
			...
			echo '<input type=hidden name=^database readonly value=\''.$d.'\'>';
			echo '<input type=hidden name=^table readonly value=\''.$t.'\'>';	
			echo '<input type=hidden name=^hv readonly value=\'h\'>';
	...
	$result=get_search_result($link,$d,$t,$post,$offset);
	echo '<table class="table table-bordered table-info table-stripped">';
	...
	echo '<tr>';
		//echo '<td>';
		//show_edit_button($link,$d,$t,$data);
		//echo '</td>';
		//show_horizontal_single_row($data);
		show_h($link,$d,$t,$data);
	echo '</tr>';
	}
	echo '</table>';
}
...
function get_search_result($link,$d,$t,$post,$offset)
{
	//my_print_r($post);	
	$fld=get_key($link,$d,$t);
	...
	{
		$sql=$sql.' where '.$sql_where.' limit '.$offset.','.$GLOBALS['limit'];
	}
	else
	{
		$sql=$sql.' limit '.$offset.','.$GLOBALS['limit'];
	}
...
function mk_search_input_from_post($link,$d,$t,$post)
{
	$fld=get_key($link,$d,$t);
	$str='';
	foreach($fld as $k=>$v)
	{	
		if(isset($post['cb_'.$v['Field']]))
		{
			$str=$str.'<input type=hidden name=\'cb_'.$v['Field'].'\'>';
			$str=$str.'<input type=hidden name=\''.$v['Field'].'\' value=\''.$post[$v['Field']].'\'>';
			if(isset($post['ex_'.$v['Field']]))
			{
				$str=$str.'<input type=hidden name=\'ex_'.$v['Field'].'\'>';
			}
		}
	}	
	return $str;
...
function show_edit_button($link,$d,$t,$data)
{
	$pk_array=get_primary_key($link,$d,$t);
	//my_print_r($pk_array);
	$pk_str_full=$d.'_'.$t.'_';
...
{
	$fld=get_key($link,$d,$t);
	echo '<tr class="bg-info"><td>Action</td>';
	foreach($fld as $k=>$v)
	{
		echo '<td>';
		echo $v['Field'];
...
function print_horizontal_header_pdf_field($link,$d,$t)
{
	$fld=get_key($link,$d,$t);
	echo '<tr align="center" ><td  width="10%"><b>Sr No</b></td><td  width="58%"><b>Topic</b></td><td  width="20%"><b>Level</b></td><td  width="12%"><b>Year</b></td>';
	echo '</tr>';
}
...
function display_staff_pdf_upload($link,$staff_id)
{
	$sql='select * from staff where id=\''.$staff_id.'\'';

	if(!$result=mysqli_query($link,$sql)){echo mysqli_error($link);return FALSE;}
	echo '<table border="1" align="center" width="100%" >
	<tr><th width="40%"><h4>ID</h4></th><th width="60%"><h4>Name</h4></th></tr>
	<tr>';
...
/*function display_staff_pdf_upload_name($link,$staff_id)
{
	$sql='select * from staff where id=\''.$staff_id.'\'';

	if(!$result=mysqli_query($link,$sql)){echo mysqli_error($link);return FALSE;}
	echo '<br><br><br><table border="1" align="right" width="100%" >
	
	<tr>';
	while($ar=mysqli_fetch_assoc($result))
	{
		echo '<td></td><td><h4>Signature of '.$ar['fullname'].'</h4></td>'.'';
	}
	echo '</tr></table>';
}*/
...
function get_dependant_table($link,$d,$t)
{
	...
	$result=run_query($link,'information_schema',$sql);
	$ar=array();
	while($one=get_single_row($result))
	{
		$ar[]=$one;
	}
...
function print_pdf($link,$d,$t,$sql)
{
	...
	echo '<table width="100%"><tr align="center" ><td><h1>Research Publication</h1></td></tr></table>';
	echo '<br><br>
	      <table width="100%" >
	      <tr align="center" ><td width="10%"></td><td width="80%"><h3>';
	       display_staff_pdf_upload($link,$_SESSION['login']);
	echo '</h3></td><td width="10%"></td></tr>
	      </table>
	       <br><br>';
	echo '<table style="width:100%;padding:10px;" border="1">';
	print_horizontal_header_pdf_field($link,$d,$t);
	//my_print_r($result);
	$counter=1;
  	while($ar=get_single_row($result))
	{	
	...
	// display_staff_pdf_upload_name($link,$_SESSION['login']);
...
//exit(0);
	...
	$pdf = new ACCOUNT('M', 'mm', 'A4', true, 'UTF-8', false);
	$pdf->SetFont('dejavusans', '', 9);
	$pdf->SetMargins(30, 20, 30);
	$pdf->AddPage();
	$pdf->writeHTML($myStr, true, false, true, false, '');
	$pdf->Output('datafile.pdf', 'I');
}
